...
from .adapters import call_external, validate_prediction
from .business import check_cases
from .generator import PROTOCOL, generate
from .metrics import aggregate, finite_sum
...
def evaluate(args):
    ...
    for seed in seeds:
        for scenario in PROTOCOL["scenarios"]:
            for origin in PROTOCOL["origins"]:
                ...
                error = None
                try:
                    forecast = validate_prediction(case.observed, call_external(args.adapter, case.observed, args.timeout))
                ...
                for item in case.observed["items"]:
                    ...
                    row_error = error
                    forecast_total = daily_error = daily_expectation_error = None
                    if sku in forecast:
                        try:
                            forecast_total = finite_sum(forecast[sku])
                            daily_error = finite_sum(abs(a-b) for a,b in zip(forecast[sku], realized, strict=True))
                            daily_expectation_error = finite_sum(abs(a-b) for a,b in zip(forecast[sku], expectation, strict=True))
                        except (ValueError, OverflowError) as exc:
                            forecast_total = daily_error = daily_expectation_error = None
                            row_error = f"NumericForecastError: {exc}"
                    ...
                    rows.append({"seed": seed, "scenario": scenario, "origin": origin, "sku": sku, "unit": item["unit"],
                                 ...
                                 "daily_absolute_error_expectation": daily_expectation_error,
    ...
    # Finite individual horizons can still overflow a pooled error total. Retain
    # all affected rows with explicit failure reasons instead of losing the run.
    aggregation_failures = []
    for unit in sorted({r["unit"] for r in rows}):
        unit_rows = [r for r in rows if r["unit"] == unit]
        checked = aggregate(unit_rows, PROTOCOL["horizon_days"])
        if checked["aggregation_error"]:
            reason = checked["aggregation_error"]
            aggregation_failures.append({"unit": unit, "error": reason, "affected_rows": len(unit_rows)})
            for row in unit_rows:
                row["error"] = row["error"] or reason
                row["forecast_quantity"] = row["daily_absolute_error"] = row["daily_absolute_error_expectation"] = None
                predictions[(row["seed"], row["origin"], row["scenario"], row["sku"])] = None
    overall = aggregate(rows, PROTOCOL["horizon_days"])
    groups = {}
    for field in ("seed", "scenario", "origin", "unit", "sku"):
        groups[field] = {str(value): aggregate((r for r in rows if r[field] == value), PROTOCOL["horizon_days"]) for value in sorted({r[field] for r in rows})}
    groups["scenario_seed"] = {f"{scenario}/{seed}": aggregate((r for r in rows if r["scenario"] == scenario and r["seed"] == seed), PROTOCOL["horizon_days"])
    ...
    report = {"schema_version": "testbed-report-v2.1", "mode": "synthetic", "split": args.split, "seeds": seeds,
              ...
              "overall": overall, "groups": groups, "rows": rows, "aggregation_failures": aggregation_failures,
    ...
    if overall["failed_rows"] or overall["aggregation_error"] or aggregation_failures or report["business"]["passed"] != len(business) or report["paired_project_checks"]["passed"] != len(checks):
        return 2
...
def markdown(report):
    ...
    lines = [f"# Testbed — {report['split']}", "", f"Synthetic; adapter `{report['adapter']}`; seeds {report['seeds']}.", "",
             ...
             f"MAE за горизонт: **{number(overall['mae_realized'])}**; MAE ожидания: **{number(overall['mae_expectation'])}**; смещение: **{percent(overall['bias_realized'])}**.",
             f"Дневной MAE: **{number(overall['daily_mae_realized'])}**; дневной WAPE: **{percent(overall['daily_wape_realized'])}**. Единицы: {', '.join(overall['units'])}.",
    ...
    for key, group in report["groups"]["scenario_seed"].items():
        lines.append(f"| {key} | {group['rows']} | {percent(group['wape_realized'])} | {number(group['mae_realized'])} | {percent(group['bias_realized'])} | {percent(group['wape_expectation'])} | {group['failed_rows']} |")
    if overall["aggregation_error"] or report["aggregation_failures"]:
        lines += ["", "Ошибки агрегации: " + str(overall["aggregation_error"] or report["aggregation_failures"])]
