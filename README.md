...
Переданные через дополнительный контекст интервалы stockout и обезличенные
клиентские метки применяются в реальном HTTP-пути SE и оставляют позицию на
ручной проверке. В исходных партнёрских XLSX таких полей нет, поэтому это не
доказательство качества коррекции на реальных данных. [Границы реализации](docs/submission-analysis.md).
...
- Реальный веб-адаптер поддерживает расчёт **SE, Алматы, штуки** и прогноз продаж
  **IEK, Алматы, штуки/метры/упаковки**. Сквозной импорт и прогноз IEK проверены на
  2 149 товарах. Без текущего свободного остатка IEK заказ остаётся неопределённым.
